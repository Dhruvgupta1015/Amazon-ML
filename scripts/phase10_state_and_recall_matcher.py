#!/usr/bin/env python3
"""
phase10_state_and_recall_matcher.py
Next-Gen Entity Resolution Engine:
1. High-Recall Clean Retrieval (max_k=120, 95.75% recall ceiling)
2. State Conflict Hard Rejection (eliminates cross-state false merges)
3. Postal Code Match / Conflict Signals
4. Alpha-Name Exact/Containment Feature Boost
5. Precomputed Scoring for Blazing-Fast Grid Search (<30s)
"""
import csv
import json
import logging
...
def score_pair_full(s1, cand):
    """
    Computes composite score with state conflict rejection and rich features.
    Returns (score, addr_jaccard, state_conflict).
    """
    # 1. State conflict
    st1, st2 = s1["state"], cand["state"]
    if st1 and st2 and st1 != st2:
        return -1.0, 0.0, True  # HARD REJECT: different states
    ...
    # Street number + postal both conflicting = very likely different businesses
    ...
    # 4-gram boost for names with good character overlap
    if ng4 >= 0.50 and jw >= 0.65:
        base = max(base, 0.50 * jw + 0.15 * tj_name + 0.15 * tj_addr + 0.20 * ng4 - 0.30 * conflict)

    # Alpha name exact match boost (e.g. Acme Corp vs Acme Inc)
    ...
    # Alpha containment boost for longer names
...
def main():
    logger.info("=" * 70)
    logger.info("PHASE 10: HIGH-RECALL CLEAN RETRIEVAL + STATE REJECTION MATCHER")
    logger.info("=" * 70)
    ...
    logger.info("Loaded %d S1 records", len(val_s1_records))

    logger.info("Loading candidates (100%% positives + 300k distractors)...")
    t0 = time.time()
    ...
    # Score all candidates with max_k=120
    logger.info("Scoring candidates (max_k=120)...")
    t0 = time.time()
    all_scored_full = {}
    ...
    logger.info("Scoring completed in %.2fs. Retrieval Recall: %.2f%% (%d/%d true pairs)",
    ...
    logger.info("Current Champion: F0.5 = %.4f | Precision = %.4f | Recall = %.4f | Singleton = %.2f%%",
                champ_f05, champion.get("precision", 0), champion.get("recall", 0),
                champion.get("singleton_accuracy", 0) * 100)

    # Grid search: tau x high_conf x addr_floor
    taus        = [0.50, 0.52, 0.54, 0.56, 0.58, 0.60, 0.62]
    high_confs  = [0.70, 0.72, 0.75, 0.78, 0.80, 0.82, 0.85]
    addr_floors = [0.08, 0.10, 0.12, 0.15, 0.18, 0.20, 0.22, 0.25]
    total_combos = len(taus) * len(high_confs) * len(addr_floors)
    logger.info("Running fast grid search over %d combinations...", total_combos)
    ...
    if passing:
        ...
        # Save report
        res_dict = {
            "run_id": "phase10-state-recall-v1",
            "model": "Phase 10: Clean Multi-Channel Retrieval (max_k=120) + State Rejection Guard",
        ...
        report_path = REPORTS_DIR / "phase10_state_recall_matcher.json"
        ...
        logger.warning("No configurations met the singleton accuracy gate (>= 90%)")
