...
fp_count = 0
print('=== EXAMINING FALSE POSITIVES (FALSE MERGES) ===')
for s1_id, s1 in list(s1_data.items())[:300]:
    true_m = gt[s1_id]
    cands = set()
    c = s1['country']
    ...
    for cid in cands:
        cand = s23_data[cid]
        pred = evaluate_pair(s1, cand)
        if pred and cid not in true_m:
            fp_count += 1
            print(f'\n[FALSE POSITIVE #{fp_count}] S1: {s1_id} ({s1["country"]})')
            print(f'   S1 Name: {s1["raw_name"]} | Clean: {s1["clean_name"]}')
            print(f'   S1 Addr: {s1["raw_addr"]} | SNum: {s1["street_num"]}')
            print(f'   Candidate {cid}:')
            print(f'   C Name:  {cand["raw_name"]} | Clean: {cand["clean_name"]}')
            print(f'   C Addr:  {cand["raw_addr"]} | SNum: {cand["street_num"]}')
            if fp_count >= 10: break
    if fp_count >= 10: break
